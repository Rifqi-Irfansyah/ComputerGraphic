...
#             py5.pop_matrix()
    ...
    def kelopak_bunga(self, Rx, Ry, angle):
        angle_step = 360 / self.jumlah_kelopak
        for i in range(self.jumlah_kelopak):
            angle = i * angle_step
...
#             transformation_matrix = np.dot(translation_matrix, np.dot(rotation_matrix, scale_matrix))
            ...
            basic.draw_ellipse(0, 0, Rx * 2, Ry * 2, self.pattern, angle)
    
    def gambar_bunga(self, value=None, name_animate=None, angle=0):
        points = 0
        ...
        if (name_animate=="skala"):
            ...
            points = self.kelopak_bunga(self.radius_lingkaran, self.radius_lingkaran // 2, angle)
...
#     rotate_matrix = tf.rotate2D(rotation_angle, x, y)
    ...
    bunga1.gambar_bunga(scale_matrix, "dwd", angle)
